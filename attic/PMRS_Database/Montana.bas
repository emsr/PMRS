...
Public Function Montana_Data_Conversion()
...
Set MyDB = CurrentDb
Set MyRS = MyDB.OpenRecordset("Montana")
...
'                    FreeSpaceLoss = 20 * (Log(MyRS("Freq")) / Log(10)) + 20 * (Log(GCDist) / Log(10)) - 27.5
                    MyRS("dbloss") = Val(Format(MyRS("dbloss"), "0.0"))
                    
                    'convert dist to km
                    MyRS("dist") = Val(Format(MyRS("dist"), "0.0"))
                    
                    'convert ht from ft to meters
                    MyRS("xht") = Val(Format(MyRS("xht"), "0.0"))
                    MyRS("rht") = Val(Format(MyRS("rht"), "0.0"))
